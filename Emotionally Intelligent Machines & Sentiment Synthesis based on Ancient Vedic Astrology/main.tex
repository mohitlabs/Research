\documentclass[12pt, a4paper]{article}
\usepackage[margin=0.5in]{geometry}
\usepackage{multicol}
\usepackage{authblk}
\usepackage[hidelinks]{hyperref}
\usepackage{cite}
\usepackage{graphicx}
\graphicspath{ {assets/img/} }
\usepackage{float}
\usepackage{amsmath}
\usepackage[siunitx, RPvoltages, american]{circuitikz}
\usepackage{polyglossia}
\setdefaultlanguage{english}
\setotherlanguage{sanskrit}
\usepackage{fontspec}
\setmainfont{Times New Roman}
\newfontfamily{\devanagarifont}[Script=Devanagari]{Lohit Devanagari}
\renewenvironment{abstract}
{\small
	\begin{center}
		\bfseries \abstractname\vspace{-.5em}\vspace{0pt}
	\end{center}
	\list{}{
		\setlength{\leftmargin}{0mm}
		\setlength{\rightmargin}{\leftmargin}
	}
	\item\relax}
{\endlist}
\title{Emotionally Intelligent Machines \& Sentiment Synthesis based on Ancient Vedic Astrology}
\makeatletter
\renewcommand\AB@affilsepx{, \protect\Affilfont}
\makeatother
\author[1]{Mohit Singh}
\author[2]{Dr. Priyank Singhal}
\author[3]{Mr. Vikas Kuchhal}
\affil[1,2,3]{College of Computing Sciences \& Information Technology\\Teerthanker Mahaveer University\\Moradabad, Uttar Pradesh\\India}
\affil[1]{\href{mailto:mohit.tca2212005@tmu.ac.in}{mohit.tca2212005@tmu.ac.in}}
\affil[2]{\href{mailto:drpriyanksinghal@gmail.com}{drpriyanksinghal@gmail.com}}
\affil[3]{\href{mailto:vikask.computers@tmu.ac.in}{vikask.computers@tmu.ac.in}}
\affil[1,2,3]{DOI:}
\begin{document}
	\pagestyle{empty}
	\includegraphics[width=\columnwidth, keepaspectratio]{PWOEExpressions}
	\newpage
	\tableofcontents
	\pagestyle{plain}
	\maketitle
	\hrule
	\begin{abstract}
		\textbf{\input{assets/Abstract}}
	\end{abstract}
	\textbf{Keywords: }
	\input{assets/Keywords}
	\newline
	\hrule
	\begin{multicols}{2}
		\section{Introduction}
		\input{assets/Introduction}
		\section{Research Model \& Hypothesis}
		\input{assets/Hypothesis}
		\section{Related Work \& Review of Literature}
		\input{assets/LiteratureReview}
		\section{Theoretical Framework}
		\subsection{What is Artificial Intelligence?}
		\input{assets/chapters/ArtificialIntelligence}
		\subsection{What is Emotional Intelligence?}
		\input{assets/chapters/EmotionalIntelligence}
		\subsection{What is Emotional Artificial Intelligence?}
		\input{assets/chapters/EmotionalArtificialIntelligence}
		\subsection{Why Truly Intelligent Machines Need Emotions?}
		\input{assets/chapters/WhyTrulyIntelligentMachinesNeedEmotions}
		\subsection{Convergence, Divergence and Belief Systems}
		\input{assets/chapters/ConvergenceDivergenceAndBeliefSystems}
		\subsection{Conscious, Subconscious \& Unconscious of AI}
		\input{assets/chapters/ConsciousSubconsciousUnconsciousofAI}
		\subsection{Logical Thinking \& Intuition}
		\input{assets/chapters/LogicAndIntuition}
		\subsection{Biology behind Emotions}
		\subsection{Emotion Dynamics}
		\input{assets/chapters/EmotionDynamics}
		\subsection{Butterfly Effect \& Chaos Theory}
		\input{assets/chapters/ButterflyEffectChaosTheory}
		\subsection{Quantum Level V/S Cosmic Level}
		\input{assets/chapters/QuantumLevelCosmicLevel}
		\subsection{Plutchik's Wheel of Emotions}
		\input{assets/chapters/Plutchik'sWheelofEmotions}
		\subsection{Ancient Vedic Astrology on Human Emotions}
		\input{assets/chapters/AncientVedicAstrology}
		\section{Methodology}
		\input{assets/Methodology}
		\section{Implementation \& Planned Experiments}
		\input{assets/Implementation}
		\section{Conclusion \& Outlook}
		\input{assets/Conclusion}
	\end{multicols}
	\hrule
	\centering
	\section*{Summary}
	\input{assets/Summary}
	\section*{Acknowledgements}
	\input{assets/Acknowledgements}
	\newline
	\hrule
	\bibliography{assets/Citations}
	\bibliographystyle{acm}
	\appendix
	\section*{Appendix}
	\input{assets/Appendix}
\end{document}